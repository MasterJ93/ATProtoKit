...
/// uploading to the AT Protocol. This allows the abstraction of both `UIImage` and
...
///
/// Implementing `ATImageProcessable` allows for the customization of image processing tasks,
/// making it useful for developers working on platforms where standard image objects like
/// `UIImage` or `NSImage` are not available, such as in Linux environments.
///
/// ### Methods
///   - `convertToImageQuery(image:altText:targetFileSize)`: Converts an image object into an
/// `ImageQuery` instance. This instance encapsulates the image data along with metadata such as the file
/// name and alternative text, preparing it for upload. It also attempts to shrink the file size
/// down to an approprate amount.
///   - `stripMetadata(from:)`: Removes EXIF and GPS metadata from the provided image data.
///   This is crucial for protecting privacy and reducing the size of the image data being transmitted.
/// - Important: `stripMetadata(from:)` is an important method to create as, according to the
/// AT Protocol documentation, the protocol may be more strict about stripping metadata in the future.\
/// \
...
/// 
/// ### Usage
/// After implementing `ATImageProcessable`, you can use your custom image processor to prepare
/// images for upload in a way that aligns with your application's requirements:
/// ```swift
/// Task {
...
public protocol ATImageProcessable {
    ...
    /// the file size to the largest possible size that doesn't exceed the limit. Note that image
