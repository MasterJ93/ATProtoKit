...
extension ATProtoKit {
    ...
    ///   - isAuthenticated: Indicates whether the method will use the access token when sending the request. Defaults to `false`.
    /// - Returns: A `Result`, containing either a ``LabelQueryLabelOutput`` if successful, ot an `Error` if not.
    public func queryLabel(uriPatterns: [String], sources: [String]?, limit: Int? = 50, cursor: String? = nil, isAuthenticated: Bool = false) async throws -> Result<LabelQueryLabelOutput, Error> {
        let accessToken = isAuthenticated == true ? "Bearer \(session.accessToken)" : nil
        ...
        do {
            ...
            let response = try await APIClientService.sendRequest(request, decodeTo: LabelQueryLabelOutput.self)
