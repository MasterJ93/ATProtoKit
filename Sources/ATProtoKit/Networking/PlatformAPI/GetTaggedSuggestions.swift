...
extension ATProtoKit {
    ...
    /// own risk.
    ///
    /// - Returns: A `Result`, containing either an ``UnspeccedGetTaggedSuggestionsOutput`` if successful, or an `Error` if not.
    public func getTaggedSuggestions() async throws -> Result<UnspeccedGetTaggedSuggestionsOutput, Error> {
        ...
        do {
            ...
            let response = try await APIClientService.sendRequest(request, decodeTo: UnspeccedGetTaggedSuggestionsOutput.self)
