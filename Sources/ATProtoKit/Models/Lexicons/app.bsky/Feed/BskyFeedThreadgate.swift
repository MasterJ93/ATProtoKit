...
///
/// - Note: According to the AT Protocol specifications: "Record defining interaction gating rules for a thread (aka, reply controls). The record key (rkey) of the
/// threadgate record must match the record key of the thread's root post, and that record must be in the same repository."
///
...
///
/// - Note: According to the AT Protocol specifications: "Allow replies from actors mentioned in your post."
...
/// A rule that indicates whether users that are on a specific list made by the post author can reply to the post.
///
    ...
    public init(from decoder: Decoder) throws {
        ...
        if let value = try? container.decode(FeedThreadgateMentionRule.self) {
            ...
            throw DecodingError.typeMismatch(EmbedViewUnion.self,
                                             DecodingError.Context(codingPath: decoder.codingPath, debugDescription: "Unknown ThreadgateUnion type"))
        }
    }
